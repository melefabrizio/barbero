<template>
  <div   class="player">
    <div class="player-controls">
      ...
      <div>
        <a v-on:click.prevent="playPausePress" title="Play/Pause" href="#">
          <svg width="18px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
            <path v-if="!this.$store.state.playing" fill="currentColor" d="M15,10.001c0,0.299-0.305,0.514-0.305,0.514l-8.561,5.303C5.51,16.227,5,15.924,5,15.149V4.852c0-0.777,0.51-1.078,1.135-0.67l8.561,5.305C14.695,9.487,15,9.702,15,10.001z"/>
    ...
    <audio :loop="innerLoop" ref="audiofile" :src="this.$store.state.audio" preload="auto" style="display: none;"></audio>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      playPausePress(){
        let current = this.$store.state.playing;
        console.log("play/pause pressed, going to "+!current)
        this.$store.commit('setPlaying', !current);
      },
      download() {
        this.stop();
      ...
      seek(e) {
        if (!this.$store.state.playing || e.target.tagName === 'SPAN') {
          return;
        }
    ...
    mounted() {
      this.$store.subscribe((mutation,state) => {
        switch (mutation.type) {
          ...
          case 'changeTrack':
            this.$store.commit('stop');
            console.log("detected change track to "+state.audio)
      ...
      //this.audio.addEventListener('pause', () => { this.$store.commit('pause') });
      //this.audio.addEventListener('play', () => { this.$store.commit('play')});
    }
  }
</script>
